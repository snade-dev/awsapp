...
export function CreateQuizPage() {
  ...
  function goNextStep() {
    if (step === 1 && (!quizForm.title.trim() || !quizForm.subject.trim())) {
      window.alert(
        "Veuillez d'abord renseigner les informations principales du quiz.",
      );
      return;
    }
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <Field label="Matiere">
                <input
                  ...
                  onChange={(event) =>
                    updateQuizForm("subject", event.target.value)
                  }
                  required
                  className={inputClassName}
                />
              ...
              <Field label="Mode par defaut">
                <select
                  value={quizForm.mode}
                  onChange={(event) =>
                    updateQuizForm("mode", event.target.value)
                  }
                  className={inputClassName}
                >
              ...
              <Field label="Duree (minutes)">
                <input
                  ...
                  onChange={(event) =>
                    updateQuizForm("duration", event.target.value)
                  }
                  required
                  className={inputClassName}
                />
              ...
              <Field label="Description" className="md:col-span-2">
                <textarea
                  ...
                  onChange={(event) =>
                    updateQuizForm("description", event.target.value)
                  }
                  className={inputClassName}
                />
              </Field>
              <label className="md:col-span-2 flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-slate-700">
                <input
                  ...
                  onChange={(event) =>
                    updateQuizForm("allowExplanations", event.target.checked)
                  }
                  className="h-4 w-4 accent-clay"
                />
                <span className="font-semibold">
                  Autoriser les explications en mode entrainement
                </span>
              </label>
            </div>
          ) : null}
            ...
            <div className="grid gap-4 rounded-[26px] border border-slate-200 bg-sand/70 p-5">
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                ...
                <button
                  ...
                  className="inline-flex min-h-11 items-center justify-center rounded-full bg-slate-900/8 px-5 py-2 font-semibold text-slate-700 transition hover:bg-slate-900/12"
                >
                  ...
                  >
                    ...
                    <Field label="Texte de la reponse">
                      <input
                        value={option.text}
                        onChange={(event) =>
                          updateDraftOption(index, "text", event.target.value)
                        }
                        className={inputClassName}
                      />
                    </Field>
              ...
              <div className="flex flex-wrap gap-3">
                <button
                  ...
                  className="inline-flex min-h-11 items-center justify-center rounded-full bg-white px-5 py-2 font-semibold text-slate-700 transition hover:-translate-y-0.5"
                >
                        ...
                        {question.options.map((option, optionIndex) => (
                          ...
                          >
                            ...
                            <p className="mt-1 text-sm text-slate-600">
                              {option.explanation || "Sans explication."}
                            </p>
                          </div>
                        ))}
                      </div>
          ...
          {step === 3 ? (
            <div className="grid gap-4">
              <article className="rounded-[26px] border border-slate-200 bg-white p-5 shadow-sm">
                <h3 className="font-display text-2xl font-bold">
                  {quizForm.title || "Quiz sans titre"}
                </h3>
                <p className="mt-2 text-slate-600">
                  {quizForm.description || "Sans description"}
                </p>
                <div className="mt-4 flex flex-wrap gap-3 text-sm text-slate-500">
                  ...
                  <span>
                    {quizForm.allowExplanations
                      ? "Explications autorisees"
                      : "Sans explications"}
                  </span>
                </div>
              </article>
              ...
              <div className="grid gap-3">
                {draftQuestions.length ? (
                  draftQuestions.map((question, index) => (
                    <article
                      key={`${question.text}-review-${index}`}
